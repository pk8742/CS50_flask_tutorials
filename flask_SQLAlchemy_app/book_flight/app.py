...
def book():
    ...
    if flight is None:
        ...
        passanger = Passanger(name=name,flight_id=flight_id)
        db.session.add(passanger)
        db.session.commit() # As far as seeing the results of your insert query, commit your transaction to persist the changes: db.session.commit()
